class CfObsBinaryBuilder::Php < CfObsBinaryBuilder::BaseDependency
  ...
  def prepare_sources
    super
    php_extensions.each do |name, metadata|
      source = metadata[:url]

      log "Downloading the sources in the package directory... (#{source})"
      File.write(metadata[:filename], open(source).read)
      verify_checksum(metadata[:filename], metadata[:md5])
    end
  end
  ...
  def extract_extensions
    ...
    Dir.mktmpdir("cf-obs-binary-builder-php") do |tmpdir|
      source = "https://raw.githubusercontent.com/SUSE/cf-public-buildpacks-ci-robots/master/binary-builds/php-extensions.yml"
      File.write(File.join(tmpdir, File.basename(source)), open(source).read)
      php_extensions_yml = YAML.load_file(File.join(tmpdir, "php-extensions.yml"))
    end
    ...
    php_extensions_yml.each do |key, elements|
      elements.each do |metadata|
        ...
        if url
          ...
          filename = metadata["name"] + "-" + metadata["version"] + suffix

          extensions[metadata["name"]] = {
